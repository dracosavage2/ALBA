...
const Dashboard: React.FC<DashboardProps> = ({ tasks, transactions, onToggleTask, onDeleteTask, onStartFocus }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {nextTask && (
        <section className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-6 rounded-3xl shadow-xl text-white relative overflow-hidden">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-white/10 rounded-full blur-2xl"></div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200 mb-2">Próximo Alarme</p>
          ...
          <div className="flex items-center space-x-3 mt-4">
            ...
            <span className="text-xs font-medium text-indigo-100 italic">hoje, {formatLocalDate(nextTask.dueDate)}</span>
          </div>
        </section>
      )}
      ...
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full uppercase">Tempo Real</span>
        </div>
            ...
            {pendingTasks.slice(0, 5).map(task => (
              <div key={task.id} className="flex items-center space-x-3 p-4 bg-slate-50 rounded-2xl border border-transparent transition-all group">
                <input 
                  ...
                  className="w-5 h-5 rounded-full border-slate-300 text-indigo-600 focus:ring-indigo-500 transition-all" 
                />
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-2 mt-0.5">
                    ...
                    {task.dueTime && (
                      <span className="flex items-center space-x-1 text-[11px] bg-indigo-600 text-white px-2 py-0.5 rounded-full font-black shadow-sm">
                ...
                <div className="flex items-center space-x-1">
                  <button onClick={() => onStartFocus(task.id, 25)} className="bg-indigo-100 text-indigo-600 p-2 rounded-lg text-[10px] font-black hover:bg-indigo-600 hover:text-white transition-all">FOCO</button>
                  {onDeleteTask && (
